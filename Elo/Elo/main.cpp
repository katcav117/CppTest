    ...
    for (size_t i = 0; i < numGames; i++)
    {
        ...
        for (size_t j = 0; j <numTournaments; j++)
        {
            ...
            if (firstWord.compare("?") == 0){
                //find past tournament
                int pastTourn = int(j)-1;
                if (j == 0){
                    pastTourn = numTournaments-1;
                }
                //display stats!
                std::cout<<"Stats for Tournament "<< pastTourn<<":"<<std::endl<<tournaments[pastTourn];
